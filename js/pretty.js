...
if (canvas) { canvas.width = canvas.clientWidth, canvas.height = canvas.clientHeight; let e = { SIM_RESOLUTION: 128, DYE_RESOLUTION: 512, DENSITY_DISSIPATION: .99, VELOCITY_DISSIPATION: .98, PRESSURE_DISSIPATION: .8, PRESSURE_ITERATIONS: 20, CURL: 20, SPLAT_RADIUS: .8, SHADING: !0, COLORFUL: !0, PAUSED: !1, BACK_COLOR: { r: 0, g: 0, b: 0 }, TRANSPARENT: !0 };

    function pointerPrototype() { this.id = -1, this.x = 0, this.y = 0, this.dx = 0, this.dy = 0, this.down = !1, this.moved = !1, this.color = [30, 0, 300] } let n = [],
        r = [];
    n.push(new pointerPrototype); const { gl: t, ext: i } = getWebGLContext(canvas);

    function getWebGLContext(e) { const n = { alpha: !0, depth: !1, stencil: !1, antialias: !1, preserveDrawingBuffer: !1 }; let r = e.getContext("webgl2", n); const t = !!r; let i, o;
        t || (r = e.getContext("webgl", n) || e.getContext("experimental-webgl", n)), t ? (r.getExtension("EXT_color_buffer_float"), o = r.getExtension("OES_texture_float_linear")) : (i = r.getExtension("OES_texture_half_float"), o = r.getExtension("OES_texture_half_float_linear")), r.clearColor(0, 0, 0, 1); const a = t ? r.HALF_FLOAT : i.HALF_FLOAT_OES; let u, v, l; return t ? (u = getSupportedFormat(r, r.RGBA16F, r.RGBA, a), v = getSupportedFormat(r, r.RG16F, r.RG, a), l = getSupportedFormat(r, r.R16F, r.RED, a)) : (u = getSupportedFormat(r, r.RGBA, r.RGBA, a), v = getSupportedFormat(r, r.RGBA, r.RGBA, a), l = getSupportedFormat(r, r.RGBA, r.RGBA, a)), { gl: r, ext: { formatRGBA: u, formatRG: v, formatR: l, halfFloatTexType: a, supportLinearFiltering: o } } }

    function getSupportedFormat(e, n, r, t) { if (!supportRenderTextureFormat(e, n, r, t)) switch (n) {
            case e.R16F:
    ...
    function supportRenderTextureFormat(e, n, r, t) { let i = e.createTexture();
    ...
    function initFramebuffers() { let n = getResolution(e.SIM_RESOLUTION),
            r = getResolution(e.DYE_RESOLUTION);
        E = n.width, S = n.height, D = r.width, y = r.height; const o = i.halfFloatTexType,
            a = i.formatRGBA,
            u = i.formatRG,
            v = i.formatR;
    ...
    function render(n) { null != n && e.TRANSPARENT ? t.disable(t.BLEND) : (t.blendFunc(t.ONE, t.ONE_MINUS_SRC_ALPHA), t.enable(t.BLEND)); let r = null == n ? t.drawingBufferWidth : D,
            i = null == n ? t.drawingBufferHeight : y; if (t.viewport(0, 0, r, i), !e.TRANSPARENT) { w.bind(); let r = e.BACK_COLOR;
    ...
    function splat(n, r, i, o, a) { t.viewport(0, 0, E, S), P.bind(), t.uniform1i(P.uniforms.uTarget, _.read.texId), t.uniform1f(P.uniforms.aspectRatio, canvas.width / canvas.height), t.uniform2f(P.uniforms.point, n / canvas.width, 1 - r / canvas.height), t.uniform3f(P.uniforms.color, i, -o, 1), t.uniform1f(P.uniforms.radius, e.SPLAT_RADIUS / 100), x(_.write.fbo), _.swap(), t.viewport(0, 0, D, y), t.uniform1i(P.uniforms.uTarget, A.read.texId), t.uniform3f(P.uniforms.color, a.r, a.g, a.b), x(A.write.fbo), A.swap() }

    function multipleSplats(e, n = !1) { for (let a = 0; a < e; a++) { const e = generateColor();
            e.r *= 10, e.g *= 10, e.b *= 10; var r = canvas.width * Math.random(),
                t = canvas.height * Math.random(),
                i = 1e3 * (Math.random() - .5),
                o = 1e3 * (Math.random() - .5);
            n && (r = canvas.width / 2, t = canvas.height / 2, i = 1e3 * (Math.random() - .5), o = 1e3 * (Math.random() - .5)), splat(r, t, i, o, e) } }

    function resizeCanvas() { canvas.width == canvas.clientWidth && canvas.height == canvas.clientHeight || (canvas.width = canvas.clientWidth, canvas.height = canvas.clientHeight, initFramebuffers()) }
    update(); var my_dx = 0,
        my_dy = 0,
        my_color = generateColor();

    function generateColor() { let e = HSVtoRGB(Math.random(), 1, 1); return e.r *= .15, e.g *= .15, e.b *= .15, e }

    function getRandomInt(e, n) { return e = Math.ceil(e), n = Math.floor(n), Math.floor(Math.random() * (n - e + 1)) + e }

    function HSVtoRGB(e, n, r) { let t, i, o, a, u, v, l, f; switch (v = r * (1 - n), l = r * (1 - (u = 6 * e - (a = Math.floor(6 * e))) * n), f = r * (1 - (1 - u) * n), a % 6) {
            case 0:
                t = r, i = f, o = v; break;
            case 1:
                t = l, i = r, o = v; break;
            ...
            case 4:
                t = f, i = v, o = r; break;
            case 5:
                t = r, i = v, o = l } return { r: t, g: i, b: o } }
    canvas.addEventListener("mousemove", e => { splat(e.offsetX, e.offsetY, 5 * (e.offsetX - my_dx), 5 * (e.offsetY - my_dy), my_color), my_dx = e.offsetX, my_dy = e.offsetY }), setInterval(function() { my_color = generateColor() }, 500), canvas.addEventListener("touchmove", e => { e.preventDefault(); const r = e.targetTouches; for (let e = 0; e < r.length; e++) { let t = n[e];
            t.moved = t.down, t.dx = 8 * (r[e].pageX - t.x), t.dy = 8 * (r[e].pageY - t.y), t.x = r[e].pageX, t.y = r[e].pageY } }, !1), canvas.addEventListener("touchstart", e => { e.preventDefault(); const r = e.targetTouches; for (let e = 0; e < r.length; e++) e >= n.length && n.push(new pointerPrototype), n[e].id = r[e].identifier, n[e].down = !0, n[e].x = r[e].pageX, n[e].y = r[e].pageY, n[e].color = generateColor() }), window.addEventListener("mouseup", () => { n[0].down = !1 }), window.addEventListener("touchend", e => { const r = e.changedTouches; for (let e = 0; e < r.length; e++)
            for (let t = 0; t < n.length; t++) r[e].identifier == n[t].id && (n[t].down = !1) }), window.addEventListener("keydown", n => { "p" === n.key && (e.PAUSED = !e.PAUSED) }) }